...
async function onFormSubmit() {
  ...
  try {
    ...
    if (isLogin.value) {
      const userCredential = await signInWithEmailAndPassword(auth, email, password as any)
      ...
      const userCredential = await createUserWithEmailAndPassword(auth, email, password as any)
      const user = userCredential.user
    ...
    toast.add({ severity: 'error', summary: 'Auth Error', detail: message, life: 3000 })
  } finally {
    isLoggingIn.value = false
    isLoading.value = false
  }
